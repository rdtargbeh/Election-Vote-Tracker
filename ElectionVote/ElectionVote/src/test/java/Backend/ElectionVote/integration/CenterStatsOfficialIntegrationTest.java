...
import Backend.ElectionVote.views.service.CenterStatsOfficialService;
...
/**
 * Integration test for v_center_stats_official; creates minimal nec_result + related tables,
 * inserts published nec_result row and asserts aggregation values through the service.
 */
@SpringBootTest
@ActiveProfiles("test")
@Testcontainers
@TestInstance(TestInstance.Lifecycle.PER_CLASS)
public class CenterStatsOfficialIntegrationTest {
    ...
    @Autowired
    private CenterStatsOfficialService service;
    ...
    void setupSchemaAndData() {
        ...
        jdbc.execute("CREATE TABLE IF NOT EXISTS polling_center_allocation (allocation_id serial PRIMARY KEY, org_id uuid, election_id uuid, center_id uuid, registered_voters int);");
        ...
        // helper functions used in your view
        ...
        // create minimal v_center_stats_official view per provided SQL (note: the view filters is_published=true)
        ...
        jdbc.update("INSERT INTO county(county_id, county_name) VALUES (?, ?)", countyId, "County P");
        jdbc.update("INSERT INTO district(district_id, district_name, county_id) VALUES (?, ?, ?)", districtId, "District P", countyId);
        jdbc.update("INSERT INTO polling_center(center_id, district_id, code, center_name) VALUES (?, ?, ?, ?)", centerId, districtId, "C001", "PC Center");
        // pca with registered_voters = 1000
        jdbc.update("INSERT INTO polling_center_allocation(org_id, election_id, center_id, registered_voters) VALUES (?, ?, ?, ?)", UUID.randomUUID(), electionId, centerId, 1000);

        // nec_result: ballots_cast 600, candidate_votes totals 580, invalid ballots = 20, published true
        String candidateVotes = "{\"" + UUID.randomUUID().toString() + "\":\"350\",\"" + UUID.randomUUID().toString() + "\":\"230\"}";
        jdbc.update("INSERT INTO nec_result(org_id, election_id, center_id, ballots_cast, candidate_votes, invalid_ballots, blank_ballots, rejected_ballots, spoiled_ballots, is_published) VALUES (?, ?, ?, ?, ?::jsonb, ?, ?, ?, ?, ?)",
                UUID.randomUUID(), electionId, centerId, 600, candidateVotes, 10, 5, 3, 2, true);

        // basic sanity select
        var rows = jdbc.queryForList("SELECT * FROM v_center_stats_official");
        assertThat(rows).isNotEmpty();
    }

    @Test
    @Transactional
    void officialCenterAggregation_returnsExpectedValues() {
        // apply GUC (if any) and then call service
        // TenantGucService must be implemented to set transaction-local GUCs; call the UUID overload explicitly
        tenantGucService.applyForTransaction((java.util.UUID) null, false, false);

        var page = service.listOfficialCenters(electionId, null, null, null, org.springframework.data.domain.PageRequest.of(0, 10));
        assertThat(page.getTotalElements()).isEqualTo(1);

        CenterStatsOfficialDto dto = page.getContent().get(0);
        assertThat(dto.getBallotsCast()).isEqualTo(600);
        assertThat(dto.getValidVotes()).isEqualTo(580);
        assertThat(dto.getInvalidTotal()).isEqualTo(20);
        assertThat(dto.getRegisteredVoters()).isEqualTo(1000);
        // turnout = 600 / 1000
        BigDecimal expectedTurnout = BigDecimal.valueOf(600d).divide(BigDecimal.valueOf(1000d), 6, BigDecimal.ROUND_HALF_UP);
        assertThat(dto.getTurnoutPct()).isEqualByComparingTo(expectedTurnout);

        // For tests where orgId is not available use evictAllStatsCaches() (evictOrgElectionCaches requires non-null orgId)
