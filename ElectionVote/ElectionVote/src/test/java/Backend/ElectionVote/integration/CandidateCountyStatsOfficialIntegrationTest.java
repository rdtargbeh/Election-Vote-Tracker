...
import Backend.ElectionVote.views.dto.CandidateCountyStatsOfficialDto;
import Backend.ElectionVote.views.service.CandidateCountyStatsOfficialService;
...
/**
 * Integration test for v_candidate_county_stats_official.
 * Builds minimal schema, creates v_candidate_center_stats_official and the county-level aggregation view,
 * seeds data and asserts aggregated values.
 */
@SpringBootTest
@ActiveProfiles("test")
@Testcontainers
@TestInstance(TestInstance.Lifecycle.PER_CLASS)
public class CandidateCountyStatsOfficialIntegrationTest {
    ...
    @Autowired
    private CandidateCountyStatsOfficialService service;
    ...
    void setupSchemaAndData() {
        ...
        // create v_center_stats_official and candidate center view (minimal)
        ...
        // create county aggregation
        jdbc.execute("CREATE OR REPLACE VIEW v_candidate_county_stats_official AS " +
                "SELECT election_id, county_id, county_name, candidate_id, candidate_name, party_id, party_name, abbreviation, SUM(candidate_votes) AS candidate_votes, SUM(ballots_cast) AS ballots_cast, SUM(center_valid_votes) AS total_valid_votes, SUM(center_invalid_total) AS total_invalid_votes, fn_pct(SUM(candidate_votes)::numeric, NULLIF(SUM(center_valid_votes),0)::numeric) AS vote_share_pct " +
                "FROM v_candidate_center_stats_official GROUP BY election_id, county_id, county_name, candidate_id, candidate_name, party_id, party_name, abbreviation;");
        ...
        jdbc.update("INSERT INTO county(county_id, county_name) VALUES (?, ?)", countyId, "County OC");
        jdbc.update("INSERT INTO district(district_id, district_name, county_id) VALUES (?, ?, ?)", districtId, "District OC", countyId);
        jdbc.update("INSERT INTO polling_center(center_id, district_id, code, center_name) VALUES (?, ?, ?, ?)", center1, districtId, "OC1", "Official Center 1");
        jdbc.update("INSERT INTO polling_center(center_id, district_id, code, center_name) VALUES (?, ?, ?, ?)", center2, districtId, "OC2", "Official Center 2");
        jdbc.update("INSERT INTO polling_center_allocation(org_id, election_id, center_id, registered_voters, ballots_cast, valid_votes, invalid_total) VALUES (?, ?, ?, ?, ?, ?, ?)",
                UUID.randomUUID(), electionId, center1, 600, 400, 390, 10);
        jdbc.update("INSERT INTO polling_center_allocation(org_id, election_id, center_id, registered_voters, ballots_cast, valid_votes, invalid_total) VALUES (?, ?, ?, ?, ?, ?, ?)",
                UUID.randomUUID(), electionId, center2, 600, 450, 440, 10);
        jdbc.update("INSERT INTO candidate(candidate_id, full_name, party_id) VALUES (?, ?, ?)", candidateId, "Official Candidate", partyId);
        jdbc.update("INSERT INTO party(party_id, party_name, abbreviation) VALUES (?, ?, ?)", partyId, "Official Party", "OFF");
        ...
        jdbc.update("INSERT INTO nec_result(org_id, election_id, center_id, ballots_cast, candidate_votes, invalid_ballots, blank_ballots, rejected_ballots, spoiled_ballots, is_published) VALUES (?, ?, ?, ?, ?::jsonb, ?, ?, ?, ?, ?)",
                UUID.randomUUID(), electionId, center1, 400, json1, 5, 3, 1, 1, true);
        jdbc.update("INSERT INTO nec_result(org_id, election_id, center_id, ballots_cast, candidate_votes, invalid_ballots, blank_ballots, rejected_ballots, spoiled_ballots, is_published) VALUES (?, ?, ?, ?, ?::jsonb, ?, ?, ?, ?, ?)",
                UUID.randomUUID(), electionId, center2, 450, json2, 5, 3, 1, 1, true);

        var rows = jdbc.queryForList("SELECT * FROM v_candidate_county_stats_official");
        assertThat(rows).isNotEmpty();
    }

    @Test
    @Transactional
    void candidateCountyOfficialAggregation_returnsExpectedValues_and_cacheEviction() {
        // anonymous context
        tenantGucService.applyForTransaction((java.util.UUID) null, false, false);

        var page = service.listCandidateCountyOfficialStats(electionId, null, null, null, org.springframework.data.domain.PageRequest.of(0, 10));
        assertThat(page.getTotalElements()).isEqualTo(1);

        CandidateCountyStatsOfficialDto dto = page.getContent().get(0);
        ...
        BigDecimal expectedShare = BigDecimal.valueOf(250d).divide(BigDecimal.valueOf(830d), 6, BigDecimal.ROUND_HALF_UP);
        assertThat(dto.getVoteSharePct()).isEqualByComparingTo(expectedShare);

        // targeted eviction by election
