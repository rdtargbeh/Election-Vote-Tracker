...
import Backend.ElectionVote.views.service.CandidateDistrictStatsOfficialService;
...
/**
 * Integration test for v_candidate_district_stats_official.
 * Builds minimal schema, v_candidate_center_stats_official and v_candidate_district_stats_official and asserts aggregated values.
 */
@SpringBootTest
@ActiveProfiles("test")
@Testcontainers
@TestInstance(TestInstance.Lifecycle.PER_CLASS)
public class CandidateDistrictStatsOfficialIntegrationTest {
    ...
    @Autowired
    private CandidateDistrictStatsOfficialService service;
    ...
    void setupSchemaAndData() {
        ...
        // create v_center_stats_official minimal
        ...
        // create candidate center and district views
        ...
        jdbc.execute("CREATE OR REPLACE VIEW v_candidate_district_stats_official AS " +
                "SELECT election_id, county_id, county_name, district_id, district_name, candidate_id, candidate_name, party_id, party_name, abbreviation, SUM(candidate_votes) AS candidate_votes, SUM(ballots_cast) AS ballots_cast, SUM(center_valid_votes) AS total_valid_votes, SUM(center_invalid_total) AS total_invalid_votes, fn_pct(SUM(candidate_votes)::numeric, NULLIF(SUM(center_valid_votes),0)::numeric) AS vote_share_pct FROM v_candidate_center_stats_official GROUP BY election_id, county_id, county_name, district_id, district_name, candidate_id, candidate_name, party_id, party_name, abbreviation;");

        // seed: two centers in same district
        ...
        jdbc.update("INSERT INTO county(county_id, county_name) VALUES (?, ?)", countyId, "County O");
        jdbc.update("INSERT INTO district(district_id, district_name, county_id) VALUES (?, ?, ?)", districtId, "District O", countyId);
        jdbc.update("INSERT INTO polling_center(center_id, district_id, code, center_name) VALUES (?, ?, ?, ?)", center1, districtId, "OC-1", "Official Center 1");
        jdbc.update("INSERT INTO polling_center(center_id, district_id, code, center_name) VALUES (?, ?, ?, ?)", center2, districtId, "OC-2", "Official Center 2");
        ...
        // candidate votes: center1 = 120, center2 = 80
        ...
        var rows = jdbc.queryForList("SELECT * FROM v_candidate_district_stats_official");
        assertThat(rows).isNotEmpty();
    }

    @Test
    @Transactional
    void candidateDistrictOfficialAggregation_returnsExpectedValues_and_cacheEviction() {
        // anonymous context
        tenantGucService.applyForTransaction((java.util.UUID) null, false, false);

        var page = service.listCandidateDistrictOfficialStats(electionId, null, null, null, null, org.springframework.data.domain.PageRequest.of(0, 10));
        assertThat(page.getTotalElements()).isEqualTo(1);

        CandidateDistrictStatsOfficialDto dto = page.getContent().get(0);
