...
import Backend.ElectionVote.views.dto.CandidateCenterStatsOfficialDto;
import Backend.ElectionVote.views.service.CandidateCenterStatsOfficialService;
...
/**
 * Integration test that builds minimal schema and asserts aggregates from v_candidate_center_stats_official.
 */
@SpringBootTest
@ActiveProfiles("test")
@Testcontainers
@TestInstance(TestInstance.Lifecycle.PER_CLASS)
public class CandidateCenterStatsOfficialIntegrationTest {
    ...
    @Autowired
    private CandidateCenterStatsOfficialService service;
    ...
    void setupSchemaAndData() {
        ...
        // v_center_stats_official (minimal)
        ...
        // create v_candidate_center_stats_official
        ...
        jdbc.update("INSERT INTO county(county_id, county_name) VALUES (?, ?)", countyId, "County Official");
        jdbc.update("INSERT INTO district(district_id, district_name, county_id) VALUES (?, ?, ?)", districtId, "District O", countyId);
        jdbc.update("INSERT INTO polling_center(center_id, district_id, code, center_name) VALUES (?, ?, ?, ?)", centerId, districtId, "OC-1", "Official Center");
        jdbc.update("INSERT INTO polling_center_allocation(org_id, election_id, center_id, registered_voters, ballots_cast, valid_votes, invalid_total) VALUES (?, ?, ?, ?, ?, ?, ?)",
                UUID.randomUUID(), electionId, centerId, 1000, 600, 580, 20);
        ...
        String candidateJson = String.format("{\"%s\":\"200\"}", candidateId.toString());
        jdbc.update("INSERT INTO nec_result(org_id, election_id, center_id, ballots_cast, candidate_votes, invalid_ballots, blank_ballots, rejected_ballots, spoiled_ballots, is_published) VALUES (?, ?, ?, ?, ?::jsonb, ?, ?, ?, ?, ?)",
                UUID.randomUUID(), electionId, centerId, 600, candidateJson, 10, 5, 3, 2, true);

        List<?> rows = jdbc.queryForList("SELECT * FROM v_candidate_center_stats_official");
        assertThat(rows).isNotEmpty();
    }

    @Test
    @Transactional
    void candidateCenterOfficialAggregation_returnsExpectedValues_and_cacheEviction() {
        // apply anonymous/explicit GUC null
        tenantGucService.applyForTransaction((java.util.UUID) null, false, false);

        var page = service.listCandidateCenterOfficialStats(electionId, null, null, null, null, null, org.springframework.data.domain.PageRequest.of(0, 10));
        assertThat(page.getTotalElements()).isEqualTo(1);

        CandidateCenterStatsOfficialDto dto = page.getContent().get(0);
        ...
        assertThat(dto.getCenterValidVotes()).isEqualTo(580);
        BigDecimal expectedShare = BigDecimal.valueOf(200d).divide(BigDecimal.valueOf(580d), 6, BigDecimal.ROUND_HALF_UP);
        assertThat(dto.getVoteSharePct()).isEqualByComparingTo(expectedShare);

        // evict caches after publish/change
